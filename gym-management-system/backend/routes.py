...
def login():
    try:
        ...
        if not data or 'username' not in data or 'password' not in data:
            logger.info(f"Login failed: Missing username or password in payload {data}")
            return jsonify({'error': 'Missing username or password'}), 400
        user = User.query.filter_by(username=data['username']).first()
        if not user:
            logger.info(f"Login failed: User {data['username']} not found")
...
def user_dashboard():
    try:
        ...
        if not user or user.role != 'user':
            logger.info(f"Unauthorized dashboard access by user_id {user_id} with role {user.role if user else 'None'}")
...
def admin_dashboard():
    try:
        ...
        if not user or user.role != 'admin':
            logger.info(f"Unauthorized admin dashboard access by user_id {user_id} with role {user.role if user else 'None'}")
            return jsonify({'error': 'Access denied'}), 403
...
def trainer_dashboard():
    try:
        ...
        if not user or user.role != 'trainer':
            logger.info(f"Unauthorized trainer dashboard access by user_id {user_id} with role {user.role if user else 'None'}")
            return jsonify({'error': 'Access denied'}), 403
...
def attendance():
    try:
        ...
        if request.method == 'GET':
            ...
            attendance = Attendance(user_id=user_id, date=datetime.now(timezone.utc).date(), attended=True)
...
def create_subscription():
    try:
        ...
        user = User.query.get(user_id)
        if user.role != 'admin':
            logger.info(f"Unauthorized subscription creation attempt by user_id {user_id}")
            return jsonify({'error': 'Access denied'}), 403
        
        duration_months = int(data.get('duration_months', 0))
        if duration_months < 1 or duration_months > 60:
            logger.info(f"Invalid duration_months: {duration_months} by user_id {user_id}")
            return jsonify({'error': 'Duration must be between 1 and 60 months'}), 400
        
        duration_days = duration_months * 30
        if duration_days > 2147483647:
            logger.info(f"Duration days {duration_days} exceeds INTEGER limit by user_id {user_id}")
            return jsonify({'error': 'Duration too large'}), 400
        
        price = float(data.get('price', 0))
        if price <= 0:
            logger.info(f"Invalid price: {price} by user_id {user_id}")
...
def manage_users():
    try:
        ...
        user = User.query.get(user_id)
        if user.role != 'admin':
            logger.info(f"Unauthorized user management attempt by user_id {user_id} with role {user.role}")
            return jsonify({'error': 'Access denied'}), 403
        if request.method == 'GET':
            users = User.query.all()
            ...
            db.session.commit()
            logger.info(f"Admin {user.username} created user {data['username']}")
            return jsonify(new_user.to_dict()), 201
        elif request.method == 'PUT':
            data = request.json
            user_to_update = User.query.get(data['id'])
            if not user_to_update:
                logger.info(f"User {data['id']} not found for update by admin {user.username}")
            ...
            db.session.commit()
            logger.info(f"Admin {user.username} updated user {user_to_update.username}")
...
def manage_trainers():
    try:
        ...
        user = User.query.get(user_id)
        if user.role != 'admin':
            logger.info(f"Unauthorized trainer management attempt by user_id {user_id} with role {user.role}")
            return jsonify({'error': 'Access denied'}), 403
        if request.method == 'GET':
            trainers = User.query.filter_by(role='trainer').all()
            ...
            db.session.commit()
            logger.info(f"Admin {user.username} created trainer {data['username']}")
            return jsonify(new_trainer.to_dict()), 201
        elif request.method == 'PUT':
            data = request.json
            trainer_to_update = User.query.get(data['id'])
            if not trainer_to_update:
                logger.info(f"Trainer {data['id']} not found for update by admin {user.username}")
            ...
            db.session.commit()
            logger.info(f"Admin {user.username} updated trainer {trainer_to_update.username}")
...
def assign_trainer():
    try:
        ...
        if admin.role != 'admin':
            logger.info(f"Unauthorized trainer assignment attempt by user_id {user_id}")
            return jsonify({'error': 'Access denied'}), 403
        data = request.json
        user = User.query.get(data['user_id'])
        trainer = User.query.get(data['trainer_id'])
        if not user or not trainer:
            logger.info(f"User {data['user_id']} or trainer {data['trainer_id']} not found")
            return jsonify({'error': 'User or trainer not found'}), 404
        if trainer.role != 'trainer':
            logger.info(f"Invalid trainer role for trainer_id {data['trainer_id']}")
            return jsonify({'error': 'Selected user is not a trainer'}), 400
        if trainer in user.trainers:
            logger.info(f"Trainer {trainer.username} already assigned to user {user.username}")
...
def register_subscription():
    try:
        ...
        user = User.query.get(user_id)
        if user.role != 'user':
            logger.info(f"Unauthorized subscription registration attempt by user_id {user_id}")
            return jsonify({'error': 'Access denied'}), 403
        data = request.json
        plan = SubscriptionPlan.query.get(data['plan_id'])
        if not plan:
            logger.info(f"Subscription plan {data['plan_id']} not found")
            return jsonify({'error': 'Subscription plan not found'}), 404
        # Check for active subscription
        active_sub = UserSubscription.query.filter_by(user_id=user_id).filter(UserSubscription.end_date > datetime.now(timezone.utc)).first()
        if active_sub:
        ...
        if existing_subscription:
            logger.info(f"User {user_id} already subscribed to plan {data['plan_id']}")
